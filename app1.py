...
API_KEY = os.environ.get("DEEPSEEK_API_KEY")  # 使用环境变量
BASE_URL = "https://api.deepseek.com/v1"
MODEL_NAME = "deepseek-chat"
MAX_ROUNDS = 3
MAX_TOKENS = 250
TEMPERATURE = 0.6
LOG_DIR = "experiment_logs_exp_group"  # 与app1一致
# =========================================
...
def get_session_data():
    ...
    if session_id not in SERVER_SESSIONS:
        # 随机分配组别
        group_assignment = random.choice(['exp', 'ctrl'])
        ...
        SERVER_SESSIONS[session_id] = {
            ...
            'group_assignment': group_assignment  # 添加组别信息
        }
...
def experiment():
    ...
    if phase == 'pre_survey' or phase == 'post_survey':
        ...
        # 兼容性检查
        if topic_category not in current_topics_config:
            for key in current_topics_config.keys():
                if topic_category in key or key in topic_category:
                    topic_category = key
                    break
        ...
        topics = current_topics_config[topic_category]['topics']
...
def download_data():
    ...
    # 添加组别字段
    writer.writerow(['session_id', 'group', 'topic_category', 'survey_type', 'scale_name', 'question_id',
    ...
    # 添加组别字段
    chat_writer.writerow(['session_id', 'group', 'topic_category', 'question', 'round', 'user_input',
