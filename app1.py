...
# --- 覆盖：从 Excel 构建“大话题/子话题”，保证与问卷完全匹配 ---
def load_big_topics_from_excel():
    # --- 优化点 1: 增加缓存检查机制 ---
    # 如果缓存文件存在，直接读取缓存，不再解析 Excel
    if os.path.exists(CACHE_PATH):
        try:
            with open(CACHE_PATH, 'rb') as f:
                print("检测到缓存，正在快速加载数据...")
                return pickle.load(f)
        except Exception as e:
            print(f"读取缓存失败: {e},将重新解析Excel")

    print("正在解析 Excel 文件，这可能需要一点时间...")
    
    # --- 优化点 2: 只读取必要的列 ---
    # 假设你的 Excel 列名包含这些，限制读取可以减少内存占用
    lib_df = pd.read_excel(EXCEL_PATH, sheet_name="lib")
    
    lib_df = lib_df.dropna(subset=["scale_name"]).copy()
    lib_df["scale_name"] = lib_df["scale_name"].astype(str).str.strip()
    lib_df["id_str"] = lib_df["id"].astype(str).str.strip()

    def sort_key(x: str):
        try:
            parts = [int(p) for p in str(x).split(".") if p.strip().isdigit()]
            return parts
        except Exception:
            return [9999, 9999]
    ...
    for scale_name, df in grouped:
        # 对每个组进行排序
        df = df.sort_values(by="id_str", key=lambda s: s.map(sort_key))
        
        # 预先提取标签（不在循环内反复检查列名）
        left_label = str(df.iloc[0].get("left_label", "立场A")).strip()
        right_label = str(df.iloc[0].get("right_label", "立场B")).strip()

        topics = []
        # --- 优化点 4: 使用 to_dict('records') 加速遍历 ---
        for idx, row in enumerate(df.to_dict('records')):
            statement = str(row.get("text", "")).strip()
            # 默认提示语优化
            question_text = str(row.get("question_text", "请讨论下列观点：")).strip()

            # 字符串构建（逻辑保持不变）
            left_prompt = f"你是一个坚定支持“{left_label}”立场的辩论者。\n\n讨论观点：{statement}\n\n要求：\n1. 坚定支持你的立场，直接回应用户观点\n2. 给出清晰的逻辑与现实例子（避免重复例子）\n3. 字数约100-150字\n4. 不要出现“作为AI”等开场白"
            right_prompt = f"你是一个坚定支持“{right_label}”立场的辩论者。\n\n讨论观点：{statement}\n\n要求：\n1. 坚定支持你的立场，直接回应用户观点\n2. 给出清晰的逻辑与现实例子（避免重复例子）\n3. 字数约100-150字\n4. 不要出现“作为AI”等开场白"
            center_prompt = f"你是一个中立的思考引导者。\n\n讨论观点:{statement}\n\n要求：\n1. 保持中立，不站队\n2. 用提问引导用户从不同角度思考\n3. 字数约80-150字\n4. 不要出现“作为AI”等开场白"

            topics.append({
                "id": idx,
                "question": statement,
                "left_stance": left_label,
                "right_stance": right_label,
                "center_stance": "思考提示",
                "intro": question_text,
                "left_system_prompt": left_prompt,
                "right_system_prompt": right_prompt,
                "center_system_prompt": center_prompt,
                "median": float(row.get('true_median', 4.0))
            })

        topics_config[scale_name] = {"id": scale_name, "topics": topics}
